...
## select {}
無限ループになる。tinygoではmain関数が終了してはいけない。プログラムが終了すると、マイコンの動作も停止してしまうのでバックグラウンド処理（割り込みなど）を継続させるために使う。

for{}でも無限ループになるがCPUを消費してしまうのでselect{}が使われる
...
SCL: 「今からデータを送るよ」のタイミング信号
SDA: 実際のデータ（0 or 1）

### SCL (Serial Clock Line)
クロック信号を送る線
マスタ(Wio Terminal)がクロックを生成
データ送信のタイミングを決める
全てのデバイスが同じクロックを共有
...
```
h := byte(0x12)  // 0001 0010 (バイナリ)

uint16(h)        // 0000 0000 0001 0010 (16bitに拡張)
uint16(h)<<8     // 0001 0010 0000 0000 (8bit左シフト)
```